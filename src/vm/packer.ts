...
export function lzwCompress(input: string): string {
  ...
  if (prev.length > 0) codes.push(dict.get(prev)!);
 ...
 */
export function streamEncrypt(data: string, key: number): string {
 ...
 */
export function streamDecrypt(data: string, key: number): string {
  const bytes = Buffer.from(data, "binary");
  const out = Buffer.alloc(bytes.length);
  for (let i = 0; i < bytes.length; i++) {
    const v = i + 1;
 ...
 */
export function bytesToHex(data: string): string {
 ...
 */
export function xorByte(byte: number, key: number): number {
  let result = 0;
  let a = byte;
  let b = key;
  let bit = 1;
  while (a > 0 || b > 0) {
    if ((a % 2) !== (b % 2)) result += bit;
    a = Math.floor(a / 2);
    b = Math.floor(b / 2);
    bit *= 2;
  }
