...
export function extractBits(value: number, start: number, width: number): number {
...
function setBits(target: number, start: number, width: number, value: number): number {
 ...
 */
export function encodeInstruction(insn: Instruction): [number, number] {
...
function writeF64(buf: number[], value: number): void {
  ...
  for (let i = 0; i < 8; i++) buf.push(tmpBuf[i]!);
}
  ...
  switch (entry.type) {
    case "string":
      ...
      if (blind && blind.kind === "str_xor") {
        ...
        for (const b of buf2) buf.push(b);
      } else {
...
export function serializeFunction(func: FuncPrototype): string {
  ...
  for (let i = 0; i < numConst; i++) {
    ...
    if (!desc) {
      ...
      writeU8(buf, klen);
      for (let k = 0; k < klen; k++) writeU8(buf, desc.key[k]! & 0xFF);
    }
  }
  ...
  for (const sub of func.subFunctions) {
    ...
    for (let i = 0; i < subBytes.length; i++) buf.push(subBytes[i]!);
  }
 ...
 */
function readU32(bytes: number[], offset: number): [number, number] {
 ...
 * Returns [value, newOffset].
 */
function readU8(bytes: number[], offset: number): [number, number] {
 ...
 * Returns [value, newOffset].
 */
function readF64(bytes: number[], offset: number): [number, number] {
  const tmpBuf = Buffer.alloc(8);
  for (let i = 0; i < 8; i++) tmpBuf[i] = bytes[offset + i]!;
 ...
 * Returns [value, newOffset].
 */
function readString(bytes: number[], offset: number): [string, number] {
  let len: number;
  ...
  for (let i = 0; i < len; i++) strBuf[i] = bytes[offset + i]!;
 ...
 */
export function deserializeFunction(data: string, offset: number = 0): [FuncPrototype, number] {
  ...
  const encB8: number[] = new Array(numInsns);
  const encB9: number[] = new Array(numInsns);
  const instructions: Instruction[] = [];
  for (let i = 0; i < numInsns; i++) {
    let b8: number, b9: number;
  ...
  // Constants
  let numConsts: number;
  [numConsts, pos] = readU32(bytes, pos);
  const constants: ConstEntry[] = [];
  ...
  let blindDescs: (BlindDesc | null)[] | undefined = undefined;
  if (isV06) {
    // numBlindEntries
    let nBlind: number;
    [nBlind, pos] = readU32(bytes, pos);
    const arr: (BlindDesc | null)[] = new Array(nBlind);
    for (let i = 0; i < nBlind; i++) {
      ...
      if (tag === 0) {
        ...
        const kArr: number[] = new Array(klen);
    ...
    for (let i = 0; i < constants.length; i++) {
      ...
      if (bd.kind === "num_split" && c.type === "number") {
        (c as { type: "number"; value: number }).value = c.value - bd.k2;
        ...
        (c as { type: "string"; value: string }).value = out;
      }
    }
  }
  ...
  // Upvalues
  let numUpvals: number;
  [numUpvals, pos] = readU8(bytes, pos);
  const upvalues: { fromStack: boolean; index: number }[] = [];
  for (let i = 0; i < numUpvals; i++) {
    let fromStack: number, idx: number;
 ...
 */
export function decodeInstruction(b8: number, b9: number): Instruction {
  const mode = extractBits(b8, 1, 2) as EncodeMode;
