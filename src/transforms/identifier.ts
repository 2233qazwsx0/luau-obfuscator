...
const RESERVED_LUA: Set<string> = new Set([
...
const KNOWN_GLOBALS: Set<string> = new Set([
 ...
 */
function collectCandidateIdents(tokens: Token[]): Set<string> {
  const declared = new Set<string>();
  const used = new Set<string>();
  const functionParams = new Set<string>();
  const localDeclCounts = new Map<string, number>();
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // track usage
    if (t.kind === TokenKind.IDENT) used.add(t.value);
  }
  ...
  for (const n of declared) {
    if ((localDeclCounts.get(n) ?? 0) === 1) safe.add(n);
  }
  ...
  const safeWithParams = new Set<string>();
  for (const n of declared) {
    if ((localDeclCounts.get(n) ?? 0) === 1) safeWithParams.add(n);
  }
...
export function renameIdentifiers(tokens: Token[], seed: number): {
  tokens: Token[];
