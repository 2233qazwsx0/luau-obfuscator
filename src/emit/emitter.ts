...
export function emit(program: Node, cipher: StringCipher): string {
  const ctx: EmitContext = { indent: 0, out: [BIT32_POLYFILL], cipher };
...
function exprToLuau(ctx: EmitContext, e: Node): string {
  switch (e.t) {
    ...
    case "String": {
      ...
      // @ts-expect-error
      const id = e.__str_id as number | undefined;
    ...
    case "Function": {
      ...
      const bodyCtx: EmitContext = { indent: ctx.indent + 1, out: [], cipher: ctx.cipher };
