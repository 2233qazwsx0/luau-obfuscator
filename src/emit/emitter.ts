...
export function emit(program: Node): string {
  const ctx: EmitContext = { indent: 0, out: [BIT32_POLYFILL], needsStringHelper: false };
...
function nl(ctx: EmitContext, n = 1): void { ctx.out.push(""); void n; }
function ind(ctx: EmitContext): string { return "    ".repeat(ctx.indent); }

function emitNode(ctx: EmitContext, n: Node): void {
  switch (n.t) {
    case "Block":
    ...
    case "If": {
      ...
      ctx.indent++; emitNode(ctx, n.branches[0]!.block); ctx.indent--;
      for (let i = 1; i < n.branches.length; i++) {
        ctx.out.push(ind(ctx) + `elseif ${exprToLuau(ctx, n.branches[i]!.cond)} then`);
        ctx.indent++; emitNode(ctx, n.branches[i]!.block); ctx.indent--;
      }
      if (n.else) {
        ctx.out.push(ind(ctx) + "else");
        ctx.indent++; emitNode(ctx, n.else); ctx.indent--;
      }
    ...
    case "While":
      ...
      ctx.indent++; emitNode(ctx, n.block); ctx.indent--;
      ctx.out.push(ind(ctx) + "end");
      return;
    case "Repeat":
    ...
    case "Function": {
      if ("name" in n && n.name) {
        ...
        ctx.indent++; emitNode(ctx, n.body); ctx.indent--;
        ctx.out.push(ind(ctx) + "end");
      }
      return;
    }
...
function exprToLuau(ctx: EmitContext, e: Node): string {
  switch (e.t) {
    case "Nil": return "nil";
    case "Bool": return e.value ? "true" : "false";
    case "Number": {
      // @ts-expect-error — meta channel for number obfuscation
      const meta = e.__obf as { kind: "bitxor"; key: number; n: number } | undefined;
    ...
    case "String": {
      ...
      const blob = e.__str_hex as string | undefined;
      // @ts-expect-error
      const key = e.__str_key as number[] | undefined;
      if (blob && key) {
    ...
    case "Table": {
      const fs = e.fields.map((f) => {
        if (!f.key) return exprToLuau(ctx, f.value);
    ...
    case "Function": {
      ...
      const bodyCtx: EmitContext = { indent: ctx.indent + 1, out: [], needsStringHelper: false };
      emitNode(bodyCtx, e.body);
      // v0.12 Feature #8: 子函数体内若引用了加密字符串，标记外层 ctx 也需要 helper。
      if (bodyCtx.needsStringHelper) ctx.needsStringHelper = true;
