...
function flattenBlock(
  ...
  for (let i = 0; i < fakeCount; i++) {
    ...
    dispatchCases.push(
      makeIf(
        makeBinop("==", makeIdent(dispatchVar), makeNumber(String(fakeState))),
        fakeBody,
      ),
    );
  }
    ...
    case "branch": {
      ...
      stmts.push(
        makeIf(
          ...
          makeBlock([trueAssign]),
          makeBlock([falseAssign]),
        ),
      );
      break;
    }
 ...
 */
function makeOpaqueTrue(v: Node, rng: () => number): Node {
 ...
 */
function makeFakeCaseBody(rng: () => number, dispatchVar: string): Node {
