...
function peek(s, off = 0) { return s.tokens[s.i + off]; }
function eat(s, kind, value) {
...
function matchKw(s, value) {
    const t = s.tokens[s.i];
    return t.kind === TokenKind.KEYWORD && t.value === value;
}
function matchOp(s, value) {
    const t = s.tokens[s.i];
    return t.kind === TokenKind.OP && t.value === value;
}
function atEof(s) { return peek(s).kind === TokenKind.EOF; }
...
function parseTypeExpr(s) {
    ...
    while (!atEof(s)) {
        const t = peek(s);
        if (depth === 0 && terminators.has(t.value))
            break;
...
function parseTypeBody(s) {
    ...
    while (!atEof(s)) {
        ...
        if (depth === 0 && t.kind === TokenKind.KEYWORD && stmtKeywords.has(t.value))
            break;
        if (depth === 0 && t.kind === TokenKind.OP && t.value === ";")
            break;
        // Track nesting
        if (t.value === "{" || t.value === "[" || t.value === "(" || t.value === "<")
            depth++;
...
function parseBlock(s, terminators) {
    const body = [];
...
function parseStmt(s) {
    ...
    if (t.kind === TokenKind.KEYWORD && t.value === "local") {
        ...
        if (matchKw(s, "function")) {
            ...
            if (node.t === "Function" && "name" in node)
                node.isLocal = true;
            return node;
        }
    ...
    if (t.kind === TokenKind.KEYWORD && t.value === "return") {
        ...
        const bare = matchKw(s, "end") || matchKw(s, "else") || matchKw(s, "elseif") ||
    ...
    if (curTok.kind === TokenKind.OP && COMPOUND_OPS.has(curTok.value)) {
        ...
        const rhs = { t: "Binop", op: binOp === ".." ? ".." : binOp, lhs: first, rhs: value, line: t.line };
...
function parseFnName(s) {
    const parts = [eat(s, TokenKind.IDENT).value];
    while (matchOp(s, ".")) {
        s.i++;
...
function parseFnDeclTail(s, _name, parts) {
...
function parseBinopThen(s) {
    return parseBinop(s, 1);
}
function parseConcat(s) {
    ...
    while (matchOp(s, "..")) {
        ...
        if (lhs.t === "Concat")
            ...
            lhs = { t: "Concat", parts: [lhs, rhs], line: lhs.line };
    }
    return lhs;
}
// Precedence climbing.
function parseBinop(s, minPrec) {
    ...
    for (;;) {
        ...
        if (!isBinopTok)
            break;
        const prec = OP_PREC[tok.value];
        if (prec === undefined || prec < minPrec)
            break;
        const op = tok.value;
        s.i++;
        const nextMin = OP_RIGHT_ASSOC.includes(op) ? prec : prec + 1;
...
function parsePrimary(s) {
    ...
    if (t.kind === TokenKind.NUMBER) {
        s.i++;
        return { t: "Number", value: t.value, line: t.line };
    }
    ...
    if (t.kind === TokenKind.KEYWORD && t.value === "true") {
        s.i++;
        return { t: "Bool", value: true, line: t.line };
    }
    if (t.kind === TokenKind.KEYWORD && t.value === "false") {
        s.i++;
        return { t: "Bool", value: false, line: t.line };
    }
    if (t.kind === TokenKind.KEYWORD && t.value === "nil") {
        s.i++;
        return { t: "Nil", line: t.line };
    }
    if (t.kind === TokenKind.OP && t.value === "...") {
        s.i++;
        return { t: "Vararg", line: t.line };
    }
    if (t.kind === TokenKind.OP && t.value === "{") {
        s.i++;
        const fields = [];
        while (!matchOp(s, "}")) {
            if (matchOp(s, "[")) {
                ...
                const name = eat(s, TokenKind.IDENT).value;
                eat(s, TokenKind.OP, "=");
                const k = { t: "String", value: name, line: peek(s).line };
                const v = parseExpr(s);
                fields.push({ key: k, value: v });
            }
            else {
...
function parseInterpString(_s, raw, line) {
    ...
    while (i < raw.length) {
        ...
        if (raw[i] === "{") {
            ...
            if (innerTokens.length > 0) {
                ...
                const innerState = { tokens: innerTokens, i: 0, errors: [] };
    ...
    if (parts.length === 0)
        return { t: "String", value: "", line };
    // Build a Concat node
    return { t: "Concat", parts, line };
}
function decodeInterpEscape(c) {
    switch (c) {
        case "n": return "\n";
        case "t": return "\t";
        case "r": return "\r";
