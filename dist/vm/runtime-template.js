 ...
 */
function genFragmentedAssembly(varName, hex, seed) {
    ...
    for (let i = 0; i < K; i++) {
        ...
        for (let j = 0; j < flen; j++)
            f += hexChars[Math.floor(rand() * 16)];
        fakeFrags.push(f);
    }
    // 存储：真+假合并后 Fisher-Yates 打乱。
    const storage = [];
    for (let i = 0; i < M; i++)
        storage.push({ hex: realFrags[i], realOrder: i });
    for (let i = 0; i < K; i++)
        storage.push({ hex: fakeFrags[i], realOrder: -1 });
    for (let i = N - 1; i > 0; i--) {
 ...
 */
export function buildRuntime(hex, cipherKey, opts = DEFAULT_RUNTIME_PROTECT, vmSeed = 0, keyHex = null) {
...
function replaceRegion(src, begin, end, replacement) {
    const re = new RegExp(`[^\\n]*--\\[\\[${begin}\\]\\][\\s\\S]*?--\\[\\[${end}\\]\\][^\\n]*\\n?`, "g");
